...
class Checker(object):
        ...
        for i, msg in enumerate(conversation):
            if msg["role"] == "system":
                ...
                assert i == 0 or i == 1 or (
                            ret[-1]["role"] in ("assistant", "function") and not ret[-1].get("function_call")), f"user error, idx {i} {ret[-1] if ret else None}"
            elif msg["role"] == "assistant" and not msg.get("function_call"):  # 打招呼会system + assistant
    ...
    def check_basic(data):
        ...
        assert "functions" in data and type(data["functions"]) is str
